...
/*
 * 曾在此处实现过「直接抓源站取正文」（自写 CSS 选择器解析），已删除。
 *
 * 初衷是想让服务端解析与 App 完全一致的 HTML，但这条路不具备通用性：
 * legado 的规则语法还包括 JS、正则、XPath、JSONPath，正文分页规律又因站而异
 * （实测某站一章分 3 页，单页只能取到 1/3 正文）。在服务端重实现一个 legado
 * 解析器，每修一处只能服务一个站，面对上千个书源注定写不完。
 *
 * 现在统一走 reader：它就是 legado 内核，分页、净化、各类规则语法全部由它
 * 处理，分段与 App 同源。残余的分段差异交由 para-anchor 的五级锚点定位吸收。
 */
