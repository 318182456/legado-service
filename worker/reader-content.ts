...
export async function fetchParagraphsFromSource(
  ...
  const steps = parseRuleChain(rule);
  if (!steps.length) return null;

  // 书源给的章节地址常是相对路径，按书源域名补全
  let target = chapterUrl;
  if (!/^https?:\/\//i.test(target)) {
    const base = String(bookSource.bookSourceUrl ?? "").replace(/#.*$/, "");
    if (!base) return null;
    try {
  ...
  try {
    ...
    const inner = applyRuleChain(html, steps);
...
/**
 * 解析 legado 的链式规则，例如 class.nr_nr@tag.div.1@textNodes。
 * 每一段是一次逐层下钻；@text / @textNodes / @html 这类取值后缀不产生下钻。
 * 出现无法处理的写法时返回空数组，交回 reader。
 */
interface RuleStep {
  kind: "class" | "id" | "tag";
  value: string;
...
function parseRuleChain(rule: string): RuleStep[] {
  const raw = rule.replace(/^@css:/i, "").trim();
  ...
  for (const seg of raw.split("@")) {
    ...
    if (!part) continue;
    // 取值后缀不参与下钻
    if (/^(text|textNodes|html|all|ownText|href|src)$/i.test(part)) continue;

    const m = part.match(/^(class|id|tag)\.([\w-]+)(?:\.(\d+))?$/i);
    if (m) {
      steps.push({
        kind: m[1].toLowerCase() as RuleStep["kind"],
        value: m[2],
        nth: m[3] !== undefined ? Number(m[3]) : undefined,
      });
      continue;
    }
    // CSS 写法的 #id / .class
    const css = part.match(/^([#.])([\w-]+)$/);
    if (css) {
      steps.push({ kind: css[1] === "#" ? "id" : "class", value: css[2] });
      continue;
    }
    // 认不出的写法一律放弃，避免解析出错误的正文
    return [];
  }
...
/** 依次下钻，返回最内层元素的 HTML */
function applyRuleChain(html: string, steps: RuleStep[]): string | null {
...
function extractStep(html: string, step: RuleStep): string | null {
  if (step.kind === "tag") {
    const all = extractAllTags(html, step.value);
    if (!all.length) return null;
    // legado 的下标从 0 起
    return all[step.nth ?? 0] ?? null;
  }
  return extractTagContent(html, step.kind, step.value);
}

/** 取出同名标签的全部内容，供带下标的规则挑选 */
  ...
  while ((m = open.exec(html)) !== null) {
    ...
    while ((inner = scan.exec(html)) !== null) {
      ...
      if (depth === 0) {
        ...
        // 从闭合标签之后继续，否则会在同一段内容里反复匹配
        resume = inner.index + inner[0].length;
        break;
      }
    }
...
function extractTagContent(html: string, attr: string, value: string): string | null {
  // [^>]* 是贪婪的，会把属性前的空白一起吞掉，故不能要求 \s 前缀
  const pattern =
    attr === "id"
      ? new RegExp(`<(\\w+)[^>]*?\\sid\\s*=\\s*["']${value}["'][^>]*>`, "i")
      : new RegExp(
          `<(\\w+)[^>]*?\\sclass\\s*=\\s*["'][^"']*\\b${value}\\b[^"']*["'][^>]*>`,
          "i"
        );
