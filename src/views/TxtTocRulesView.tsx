...
import { RefreshCw, Plus, Trash2, List, MoreVertical, Pencil, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function TxtTocRulesView({ onAdd, onEdit }: TxtTocRulesViewProps) {
  ...
  const fetchRules = async (q = '', p = 1) => {
    setLoading(true);
    try {
      const data = await api.getTxtTocRules(q, p);
      setRules(data.rules);
      setTotal(data.total);
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="px-4 py-3 border-t border-outline-variant/30 flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-container-lowest">
          <div className="text-xs text-secondary font-medium">
            共 <span className="text-on-surface font-bold">{total.toLocaleString()}</span> 条目录规则
