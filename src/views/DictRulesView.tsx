...
export default function DictRulesView({ onAdd, onEdit }: DictRulesViewProps) {
  ...
  const handleToggle = async (id: number, enabled: boolean) => {
    try {
      await api.toggleDictRule(id, !enabled);
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-surface text-secondary text-xs font-bold uppercase tracking-wider border-b border-outline-variant">
                <th className="py-3 px-6">字典名称</th>
                <th className="py-3 px-6">划词查询 URL 规则 / 排序号 / 内容解析展示</th>
                  ...
                  <tr key={rule.id} className="border-b border-outline-variant/30 hover:bg-surface-container-low transition-colors group">
                    ...
                    <td className="py-4 px-6 max-w-md">
                      <div className="flex flex-col gap-1.5">
                        <code className="text-[10px] bg-surface-container px-2 py-1 rounded text-secondary font-mono break-all leading-relaxed" title={rule.url_rule}>{rule.url_rule}</code>
                        <div className="flex flex-wrap gap-2">
                          <span className="text-[9px] bg-tertiary/10 text-tertiary font-bold px-2 py-0.5 rounded">
                            排序: {rule.sort_number}
                          </span>
                          {rule.show_rule && (
                            <span className="text-[9px] bg-secondary/10 text-secondary font-mono px-2 py-0.5 rounded break-all max-w-[250px]" title={rule.show_rule}>
                              解析: {rule.show_rule}
                            </span>
                          )}
                        </div>
